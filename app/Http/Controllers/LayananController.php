...
class LayananController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $main_title = "Layanan";
        $data = Layanan::where(function ($query) use ($keyword) {
                $query->orWhere('name', 'LIKE', '%' . $keyword . '%');
            })
            ->orderBy('created_at', 'desc')
            ->paginate(10);
        $data->withPath('layanan');
        $data->appends($request->all());

        return view('admin.master.layanan.index', compact('main_title', 'data', 'keyword'));
    }

    public function add(Request $request)
    {
        $main_title = "Layanan";
        return view('admin.master.layanan.add', compact('main_title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('img_name')) {
            ...
            $request->file('img_name')->move(public_path('file/img/layanan'), $filename);
            $send  = Layanan::create([
                'name'          => $request['name'],
                'desc'          => $request['desc'],
                'img_name'      => $filename,
                'img_path'      => 'file/img/layanan/',
            ]);

            if ($send) {
                return redirect()
                    ->route('master.layanan.add')
                    ->with([
    ...
    {
        $hapus = Layanan::findOrFail($request->id);
        if ($hapus->img_name) {
