                    ...
                    <div class="card-header">
                        ...
                        <div class="row">
                            <div class="col-md-3 mt-5">
                                <form action="{{ route('master.artikel') }}" method="GET">
                            ...
                            <div class="col-md-9 mt-5 text-right">
                                <a href="{{ route('master.artikel.add') }}" class="btn btn-primary mdi mdi mdi-magnify px-3"
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" width="15%">Thumbnile</th>
                                        <th class="text-center" width="15%">Kategori</th>
                                        <th class="text-center" width="20%">Judul</th>
                                        <th class="text-center" width="10%">Tanggal</th>
                                        <th class="text-center" width="10%">Like</th>
                                        <th class="text-center" width="10%">Status</th>
                                        <th width="20%"></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($data as $key=> $artikel)
                                        <tr>
                                            <th class="text-center">{{ $key + 1 }}</th>
                                            <td class="text-center">
                                                <img src="{{ URL($artikel->thumbnail_path.$artikel->thumbnail_name)}}" alt="logo.png" width="80px">
                                            </td>
                                            <td class="text-center">{{ $artikel->nama_kategori }}</td>
                                            <td>{{ $artikel->title }}</td>
                                            <td class="text-center">{{ date('d F Y', strtotime($artikel->created_at)); }}</td>
                                            <td>

                                            </td>
                                            <td  class="text-center">
                                                @if ($artikel->st_publish == '1')
                                                    <span class="badge badge-pill badge-success">Publish</span>
                                                @else
                                                    <span class="badge badge-pill badge-secondary">Tidak Publish</span>
                                                @endif
                                            </td>
                                            <td class="text-center">
                                                <a href="{{ route('master.artikel.edit', $artikel->id) }}" title="edit"
                                                    class="btn waves-effect waves-light btn-out btn-warning rounded"><i
                                                        class="fa fa-edit"></i></a>
                                                <a href="#" onclick="hapus({{ $artikel->id }})" title="Hapus"
                                    ...
                                    @empty
                                        <tr>
                                            <td class="text-center text-mute" colspan="6">Data tidak tersedia !
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
            ...
            function hapus(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.get('/master/artikel/delete/' + id)
